...
class TwoPopulationsIFEnsembleDensitySimple2Integrator:
    ...
    def integrate(self, eRho0, iRho0, wEI, wIE, eInputs):
        ...
        for n in xrange(1, eInputs.size):
            ...
            eQ = self._a0+\
                  (eInputs[n]+\
                    self._nInputsPerNeuron*\
                    self._fracExcitatoryNeurons*eRs[n-1])*self._a1-\
                  (wIE*iRs[n-1]+\
                    self._nInputsPerNeuron*(1-self._fracExcitatoryNeurons)*\
                    eRs[n-1])*self._a2
            iQ = self._a0+\
                  (wEI*eRs[n-1]+\
                    self._nInputsPerNeuron*\
                    self._fracExcitatoryNeurons*iRs[n-1])*self._a1- \
                    self._nInputsPerNeuron*(1-self._fracExcitatoryNeurons)*\
                    iRs[n-1]*self._a2
