...
#     tf = 1.0
    ...
    scaleE = 1500
    freq = 4
    def rSinusoidalInputMeanFrequency(t, scale,
                                        omega=2*np.pi*freq, phase=np.pi):
        r = scale*np.sin(omega*t+phase)
        if r<0:
            r = 0.0
        return(r)
    ...
    eStim = lambda t: rSinusoidalInputMeanFrequency(t=t, scale=scaleE,
                                                         omega=2*np.pi*freq)
    ...
    diffMatrix = a0+max(sigma0E, minSigmaE)*a1-sigma0I*a2
    eigenRepos = EigenReposForTwoStim()
    eVals, eVecs, _, _, _, _ = \
     eigenRepos.computeEigenDecomposition(diffMatrix=diffMatrix, prevEVecs=None)
...
#     coefsFromDE[1] = 0.0
    ...
    axarr[1].plot(ts, spikeRatesFromRho, label="Full", color="blue")
...
#     pdb.set_trace()
    ...
    for i in xrange(1, len(ts)):
        ...
        diffMatrixFromRhos = a0+sigmaEFromRho*a1-sigmaIFromRho*a2
        rho = (identityHD+dt*diffMatrixFromRhos).dot(prevRho)
        rhosCol[:, i] = rho
        prevRho = rho

        diffMatrixFromDE = a0+max(sigmaEFromDE, minSigmaE)*a1-sigmaIFromDE*a2
        eVals, eVecs, _, _, _, _ = \
         eigenRepos.computeEigenDecomposition(diffMatrix=diffMatrixFromDE,
                                               prevEVecs=None)
