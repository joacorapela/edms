...
def main(argv):
    ...
    if(len(argv)==4):
        ...
        print("Pausing at %f seconds" % timeToPause)
...
#     scaleI = 200
    ...
    b = 0.6
#     def sinusoidalInputMeanFrequency(t, scale, omega, b=b, phase=-np.pi/2):
    def sinusoidalInputMeanFrequency(t, scale, omega, b=b, phase=0):
        return(scale*(1+b*np.sin(omega*t+phase)))

    def constantInputMeanFrequency(t, scale):
        return(scale)

    linearStimSlope = 1
    def linearInputMeanFrequency(t, scale, slope=linearStimSlope):
        return(scale+t*linearStimSlope)

    eStim = lambda t: sinusoidalInputMeanFrequency(t=t, scale=scaleE,
                                                        omega=2*np.pi*freqE)
    iStim = lambda t: sinusoidalInputMeanFrequency(t=t, scale=scaleI,
                                                        omega=2*np.pi*freqI)
    ...
    # coefsFromDE[0]=1.0 (see Knigth00 Eq. 6.16)
    coefsFromDE[0] = 1.0
    coefsFromDE[1] = 0.0
    ...
    axarr[0].plot(vs, rhoFromDE, label=r"$\rho$ (%d mb)" % nEigen, color="red")
    ...
    axarr[1].plot(ts, spikeRatesFromDE, label="r (%d mb)" % nEigen, color="red")
...
#     pdb.set_trace()
    ...
    for i in xrange(1, len(ts)):
        t = ts[i]
        if ts[i]>timeToPause and not alreadyPaused:
            alreadyPaused = True
            pdb.set_trace()
        if i%1000==0:
        ...
        # To avoid loosing probability, lets enforce 
        # coefsFromDE[0]=1.0 (see Note 12 in knigth00)
        coefsFromDE[0] = sumRho0/sum(rEVecs[:, 0].real)
        coefsFromDE[1] = 0
...
#         pdb.set_trace()
        ...
        if i%100==0:
            ...
            axarr[0].plot(vs, rhoFromDE, label=r"$\rho$ (%d mb)" % nEigen,
                              color="red")
            axarr[0].set_xlabel("Normalized Voltage")
            axarr[0].set_ylabel(r"$\rho$")
            ...
            axarr[1].cla()
            axarr[1].plot(ts, spikeRatesFromRho, label="r (full)", color="blue")
            axarr[1].plot(ts, spikeRatesFromDE, label="r (%d mb)" % nEigen,
                              color="red")
            axarr[1].set_xlabel("Time (sec)")
            axarr[1].set_ylabel("Firing Rate (ips)")
            axarr[1].set_ylim(bottom=0, top=50)
